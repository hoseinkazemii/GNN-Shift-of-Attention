...
log_file    = f"./logs/linear_baseline_log_{timestamp}.log"
...
class FlattenedLinearBaseline(nn.Module):
    def __init__(self, input_dim, p_dropout=0.3):
        super().__init__()
        self.fc = nn.Sequential(
            nn.Flatten(),                     # flatten [B, WIN, N_OBJ*F] → [B, WIN*N_OBJ*F]
...
model  = FlattenedLinearBaseline(input_dim=INPUT_DIM).to(device)
logging.info(f"\n{model}\nTotal parameters: "
...
plt.title("Linear Baseline Confusion Matrix (Test)"); plt.tight_layout()
plt.savefig("./plots/linear_baseline_confusion_matrix_test.png", dpi=300)
logging.info(f"Confusion matrix:\n{cm}")
logging.info("\nClassification Report (Linear Baseline, Test):\n"+
...
def plot_metric(idx, label, ylabel):
    ...
    plt.savefig(f"./plots/linear_baseline_{label.lower().replace(' ','_')}.png", dpi=300)
    plt.close()
